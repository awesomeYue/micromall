...
public class ProductServiceImpl implements ProductService {
  ...
  public ServerResponse<PageInfo> searchProduct(
    ...
    if (StringUtils.isNotBlank(productName)) {
      productName =
          new StringBuilder().append("%").append(productName.trim()).append("%").toString();
    }
  ...
  @Override
  public ServerResponse<ProductDetailVo> getProductDatil(Integer productId) {
    if (productId == null) {
      return ServerResponse.createByErrorCodeMessage(
          ResponseCode.ILLEGAL_ARGUMENT.getCode(), ResponseCode.ILLEGAL_ARGUMENT.getDesc());
    }
    ...
    if (product.getStatus() != Const.ProductStatusEnum.ON_SEAL.getCode()) {
      return ServerResponse.createByErrorMessage("产品已下架或者删除");
    }
    ProductDetailVo productDetailVo = assembleProductDetailVo(product);
    return ServerResponse.createBySuccess(productDetailVo);
  ...
  @Override
  public ServerResponse<PageInfo> getProductByKeywordOrCategoryId(
    ...
    if (categoryId != null) {
      ...
      if (category == null && StringUtils.isBlank(keyword)) {
        // 没要改分类，并且没有关键字，这个时候返回一个空的结果集，不报错
        PageHelper.startPage(pageNum, pageSize);
        List<ProductListVo> productListVoList = Lists.newArrayList();
    ...
    PageHelper.startPage(pageNum, pageSize);
    // 排序处理
    if (StringUtils.isNotBlank(orderBy) && Const.ProductOrderBy.PRICE_ASC_DESC.contains(orderBy)) {
